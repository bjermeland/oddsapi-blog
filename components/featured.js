...
export default function Featured({ post, pathPrefix }) {
  ...
  return (
    <div
      className={cx('grid md:grid-cols-2 gap-5 md:gap-10 md:min-h-[calc(100vh-30vh)]')}
      style={{
              ...
              <div className="flex flex-col gap-3 md:items-center md:flex-row">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-gray-100 ">
                    {post.author.name} <span className="hidden pl-2 md:inline"> ·</span>
                  </p>
                </div>

                <div>
                  <div className="flex space-x-2 text-sm md:flex-row md:items-center">
                    <time className="text-white" dateTime={post?.publishedAt || post._createdAt}>
                      {format(parseISO(post?.publishedAt || post._createdAt), 'MMMM dd, yyyy')}
                    </time>
                    <span className="text-white">· {post.estReadingTime || '5'} min read</span>
                  </div>
                </div>
              </div>
            </div>
